...
public class step7_quote {
    ...
    public static MalVal quasiquote(MalVal ast, Env env) {
        if (!(ast instanceof MalList))
            return ast;

        if (starts_with(ast, "unquote"))
            return EVAL(((MalList)ast).nth(1), env);

        List<MalVal> lst = new ArrayList<MalVal>();
        for (Integer i=0; i<((MalList)ast).size(); i++) {
            MalVal elt = ((MalList)ast).nth(i);
            if (starts_with(elt, "splice-unquote"))
                lst.addAll(((MalList)EVAL(((MalList)elt).nth(1), env)).value);
            else
                lst.add(quasiquote(elt, env));
        }
        if (ast instanceof MalVector)
            return (MalVal)new MalVector(lst);
        else
    ...
    public static MalVal EVAL(MalVal orig_ast, Env env) throws MalThrowable {
        ...
        case "quote":
            return ast.nth(1);
        case "quasiquote":
            return quasiquote(ast.nth(1), env);
        case "do":
